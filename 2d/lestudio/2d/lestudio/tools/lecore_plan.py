...
FACTS = [
    ...
    ("what is the status of R74 step 02 the gradient tool",
     "SHIPPED (R74b). Document.gradient journals a pixel-free {op:gradient} record (points, kind, stops, opacity, dither, seed, frozen gate asset) and _gradient_layer is the ONE applier the tool and the replay branch both call -- the pwarp model. Kinds: linear, radial, angle, reflected, diamond. Stops are sorted and cleaned in _gradient_ramp so any client's order replays the same. Dither is seeded so replay reproduces. /api/gradient validates finite points, the kind and the stops list. Client: tGradient (Shift+G, cycles with the bucket on G), a ramp-line preview with a handle at each end, Shift snaps to 45 degrees, gradHud with kind/from/to/swap/to-transparent/opacity/dither. Tests: 10 in test_r74.py, 21 in test_gradient_ui.js."),
    ("what trap did step 02 surface",
     "tests/domshim.js had NO checked, disabled or onchange support: el.checked was undefined on every shim element, so client code reading .checked passed its tests while posting undefined to the server, and a change handler never ran. The shim now mirrors the checked/disabled attributes and dispatches change and input like click. Any earlier shim test that set .checked by hand was only passing by accident."),
    ("how do journaled non-stroke ops work in leStudio",
     "The pwarp model: record_stroke with a brush dict {op: NAME, ...geometry, sel_asset, sel_invert} and no pixels, add the layer to the undo entry's rerender list, and put ONE applier function on Document that both the tool and the replay branch (in the op dispatch beside pwarp/xform/gradient) call. That is how a tool stays replayable, undoable as one entry, and .lews-portable without storing an image."),
    ("what is the status of R74 step 03 stroke and fill selection",
     "SHIPPED (R74c). selection_outline traces the gate with cv2 findContours + approxPolyDP (NOT scikit-image, which is optional in this app and absent on many installs -- a test pins that). stroke_selection paints each ring as one ordinary journaled stroke carrying the brush's media, with inside= to clip to the selection; that IS the rectangle/ellipse/polygon shape tool and the result is editable paint. fill_selection journals pixel-free by reusing the gradient record with one flat stop. Routes /api/stroke_selection (returns rings + a warning when nothing is selected) and /api/fill_selection. Select panel: Stroke outline, Fill, inside. Brush and eraser: Shift-drag is a straight line, Shift+Alt locks to 45. Ctrl+A, Ctrl+E, Ctrl+Shift+E, Ctrl+Shift+N, Ctrl+T added and listed in the shortcut sheet. Tests: 9 in test_r74.py, 22 in test_shape_ui.js."),
    ("what trap did step 03 surface",
     "The straight-line branch called drawStrokePreview -- a function that does not exist anywhere; node --check accepted it because it is a valid call. The real preview is drawLocalStroke. This is precisely the phantom-function trap in engineering-notes, and it was caught only because the shim test LIFTS the branch out of the shipped handler and RUNS it. A text-matching test would have passed."),
    ("why did the creature brush have no UI",
     "Two faults at once (R74d). Its BUTTON was the fourth in the collapsed generator group -- .tgw>button:not(.cur) is display:none and the visible face is Scribble -- and its KEY was a lie: R74a had made the creature the far end of a three-tool Shift+L cycle (lasso -> polygon lasso -> creature) while the tooltip said Creature (Shift+L), so from any other tool Shift+L reached the polygon lasso. With no visible button and no working key the panel could not be opened at all. Fix: the creature is its own solo tgrp with an always-visible cur button, key A; L/Shift+L is the lasso family; D/Shift+D is the generator family (scribble, hatch, textile). Scribble/Hatch/Textile had advertised (G), (D), (W) since before R74 -- the bucket, nothing, and the wand."),
    ("what is the rule about tool buttons and keys in leStudio",
     "A toolbar that shows a key which does not work is worse than one that shows none. Two places advertise a tool's key -- the button tooltip and the status-line tip in the tips object -- and both must agree with TOOLKEY and SHIFTTOOLKEY. A tool must be reachable in ONE press, never by cycling through two others. And a button without class cur inside a multi-button .tgrp is INVISIBLE until the group is opened, so any new tool either gets cur or its own solo group. test_r74_every_tool_key_a_tooltip_advertises_actually_works walks all of it."),
