...
its walk as ONE journaled stroke. `rules` is a dict of wants in -1..1
(`lines`, `self`, `others`, `light`, `color` + `target` RGB, `field` 0..1,
`wander` 0..1, `solid` bool); `random_rules: true` draws one and echoes it.
`poly` (inline polygon) or `selection` makes it a FILL: creatures are released
spread through the region and cannot leave it. `path` releases them along a
drag; `heading` faces them. They carry `media`/`material`/`load` like any
stroke. Time is steps at 60/s, not wall clock, so `seed` reproduces. The
walk is plain paint afterwards: nudge, restyle, undo all apply.
