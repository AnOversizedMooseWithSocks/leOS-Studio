...
def creature():
    """R73 creature brush: {"layer", "x", "y", "seconds" (3), "creatures"
    1..10, "radius", "color", "opacity", "hardness", "rules" {lines, self,
    others, light, color, field, wander, target}, "random_rules", "field"
    none|curl|vortex, "speed", "turn", "vary_color", "seed"?, "path"?,
    "selection"?, "sel_invert"?, "feather"?, "sample", "record"}. Up to ten
    leCore CreatureMinds explore the layer as a map and each paints its
    walk as one ordinary journaled stroke. Returns {ok, strokes, steps,
    seed, rules, seconds} -- the rules echoed so a random draw can be seen
    and kept."""
    d = request.json or {}
    try:
        ...
        r = DOC.creature_paint(
            d["layer"], float(d["x"]), float(d["y"]),
            seconds=float(d.get("seconds", 3.0)),
            creatures=int(d.get("creatures", 1)),
            ...
            rules=rules,
            random_rules=bool(d.get("random_rules")),
            field=str(d.get("field", "curl")),
            speed=float(d.get("speed", 1.0)),
            turn=float(d.get("turn", 1.0)),
            vary_color=float(d.get("vary_color", 0.0)),
            seed=d.get("seed"),
            path=_clean_path(d.get("path")),
            selection=d.get("selection") or None,
            sel_invert=bool(d.get("sel_invert")),
            feather=float(d.get("feather", 0.0)),
            sample=str(d.get("sample", "layer")),
