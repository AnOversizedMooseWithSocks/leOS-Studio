...
class Element {
  ...
  // R74: checkboxes and change/input handlers. Without these `el.checked` was
  // undefined on every shim element -- so a test could only pass by setting
  // the property by hand, and code reading `.checked` looked fine while
  // posting `undefined` to the server. `checked` now mirrors the attribute
  // the markup carries, and onchange/oninput dispatch like onclick.
  get checked() {
    if (this._checked === undefined) this._checked = this.attrs.checked !== undefined;
    return !!this._checked;
  }
  set checked(v) { this._checked = !!v; }
  get disabled() {
    if (this._disabled === undefined) this._disabled = this.attrs.disabled !== undefined;
  ...
  dispatch(type, ev) {
    ...
    for (let i = 0; i < path.length && !ev._stop; i++) {
      ...
      if (type === 'change' && path[i]._onchange && !ev._stop) path[i]._onchange.call(path[i], ev);
      if (type === 'input' && path[i]._oninput && !ev._stop) path[i]._oninput.call(path[i], ev);
    }
    return ev;
  }
}
