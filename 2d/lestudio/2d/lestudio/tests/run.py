...
def main(argv=None):
    ...
    # R73: five round files (r58-r60, r63, r64) are not in the public
    # checkout. A missing module used to abort the whole runner with a
    # traceback before a single test ran; now it is reported and skipped,
    ...
    for m in ("test_studio", "test_r4", "test_r5", "test_r6",
        ...
        try:
            ...
            if e.name != m:
                raise
            print("note: %s not in this checkout -- skipped" % m)
    by_name = {}
    for m in mods:
        for n in dir(m):
