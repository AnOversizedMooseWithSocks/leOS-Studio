...
Source: https://github.com/AnOversizedMooseWithSocks/leOS-Studio, folder `3d/lestudio3d` (the 3-D sibling of
`2d/lestudio`). Engine: https://github.com/AnOversizedMooseWithSocks/leCore (`pip install leos-core`).
...
### The engine
leStudio3d builds on **leCore**, installed from PyPI as `leos-core` (import name `lecore`); nothing is vendored.
`Help ▸ Engine status` is the engine's own `engine_status()` (version, faculties, extras, GPU, determinism policy)
plus this app's `features()` gate, which names any faculty the installed build lacks.
Opens at http://127.0.0.1:5000/.

## Layout (professional-app style)
- **Menu bar** (top): File / Create from / Add / Generate / Select / Mesh (incl. Remesh: retopo · CVT · manifold · orient) / Modifiers / Window / Help.
...
- **Shared workspace (`.lews`).** *File ▸ Open shared workspace .lews (leStudio)…* opens a workspace written
  by **leStudio**, the 2-D image editor built on the same engine. leStudio3d reads the painter's documents,
  ...
  *File ▸ Export shared workspace .lews* writes your objects back into the same file as `lestudio3d.object`
...
The engine is the `leos-core` package; this folder holds only the app.
