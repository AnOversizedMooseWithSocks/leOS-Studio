"""leStudio3d -- a C4D-style polygon modeller on the leCore mesh kernel (Blueprint, routes at /api/...).
...
bp = Blueprint("lestudio3d", __name__)
...
def _primitive(name):
    ...
    import holographic.mesh_and_geometry.holographic_mesh as hm
...
def _mint_id():
    """An object id. ADOPTED (1.6.0, APP_FOUNDATION 2a): with a live .lews open the id comes from the
    engine's persisted per-prefix counter (`Workspace.mint("O")` -- journalled, collision-free across
    apps and processes, deterministic on replay); without one, the session counter as before. Ids are
    kept on load now (see scene_save/scene_load), so an agent no longer re-reads the scene after open."""
    ws = _workspace()
    if ws is not None:
        try:
            oid = str(ws.mint("O"))
...
def _add_object(name, mesh, mats=None, sdf_tree=None, kernel_src=None, oid=None):
    if oid is None or str(oid) in _S["objects"]:
        oid = _mint_id()
    oid = str(oid)
...
def _transfer_uv_resid(mt_mod, src_mesh, src_uv, tgt_verts):
    """transfer_uv -> (uv, median residual): the residual is the honest error signal the GLB report shows."""
    uv, dist = mt_mod.transfer_uv(src_mesh, np.asarray(src_uv, float), np.asarray(tgt_verts, float))
...
def _material_names():
    ...
    return sorted(set(_matlib().names()) | set(_CUSTOM_MATS))
...
@bp.route("/api/scene")
def scene_get():
    """Current scene: objects with ids, names, face/vertex counts, materials, transforms. ?view=summary returns
    ids / names / bbox / counts / unique materials only -- the full payload is ~1 MB for a 3000-face scene."""
    _init()
    with _LOCK:
        return jsonify(_payload(view="summary" if request.args.get("view") == "summary" else "full"))
...
def _eval_expr_batch(expr, pts):
    ...
    from holographic.io_and_interop.holographic_milkdrop import MilkExpr
...
def _mesh_verb(m, name, d):
    ...
    if name == "extrude":
        ...
        import holographic.mesh_and_geometry.holographic_meshverbs2 as _mv2
        verts = d.get("verts")
        ...
        import holographic.mesh_and_geometry.holographic_deform as _df
        V = m.vertices
        ...
        import holographic.mesh_and_geometry.holographic_crossfield as cf
        from holographic.mesh_and_geometry.holographic_mesh import Mesh
        tri = m
...
def op():
    ...
    with _LOCK:
        ...
        if name == "bridge":
            ...
            if o is None:
                return jsonify({"error": "no such object"}), 400
            import holographic.mesh_and_geometry.holographic_meshselect as msel
        ...
        if name == "boolean":
            ...
            try:
                if rad == 0.0 and kind not in ("smooth", "chamfer") and o.sdf_tree is not None and other.sdf_tree is not None:
                    ...
                    from holographic.mesh_and_geometry.holographic_fillet import fillet_union, fillet_intersection, fillet_difference
                    from holographic.mesh_and_geometry.holographic_meshbridge import marching_tetrahedra_vec
        ...
        if name == "lathe":
            ...
            from holographic.mesh_and_geometry.holographic_sdf2d import polygon2d, revolve
        ...
        if name in ("retopo", "cvt_remesh", "manifold_cleanup", "orient"):
            # ADOPTED (1.6.0): four engine mesh doors the catalog grew since 0.2.11, each a Mesh-menu op:
            #   retopo          m.surface_retopo   field-aligned quads, vertices never leave the surface
            #   cvt_remesh      m.cvt_remesh       Lloyd-relaxed isotropic decimation (CWF)
            #   manifold_cleanup m.manifold_cleanup split non-manifold fins so QEM/half-edge accept the mesh
            #   orient          m.mesh_orient      consistent face winding (the precondition for field work)
            # Materials transfer by nearest centroid, like decimate. The engine's report rides in the reply.
            o = _S["objects"].get(oid)
            if o is None:
                return jsonify({"error": "no such object"}), 400
            m = o.mesh
            ...
            try:
                if name == "retopo":
                    # the catalog's stated precondition: consistent winding (a marching-tetra + decimate
                    # mesh routinely fails is_oriented) -- orient first, do not ask the user to know that
                    for prep in (mind.manifold_cleanup, mind.mesh_orient):
                        try:
                            o_ = prep(tri)
                            tri = o_[0] if isinstance(o_, tuple) else o_
                        except Exception:
                            pass
                    res, rep_ = mind.surface_retopo(tri, density=float(np.clip(d.get("density", 1.0), 0.2, 4.0)),
                                                    fast=bool(d.get("fast", True)))
                elif name == "cvt_remesh":
                    res, rep_ = mind.cvt_remesh(tri, n_sites=int(np.clip(int(d.get("sites", 400)), 20, 20000)),
                                                iterations=int(np.clip(int(d.get("iterations", 4)), 1, 12)))
                elif name == "manifold_cleanup":
                    res = mind.manifold_cleanup(tri, keep_largest=bool(d.get("keep_largest", True)))
                    if isinstance(res, tuple):
                        res, rep_ = res
                    else:
                        rep_ = {}
                else:
                    res = mind.mesh_orient(tri)
                    if isinstance(res, tuple):
                        res, rep_ = res
                    else:
                        rep_ = {}
            except Exception as e:
                return jsonify({"error": f"{name} refused: {type(e).__name__}: {e}"}), 400
            new_mesh = Mesh(np.asarray(res.vertices, float), [tuple(int(v) for v in f) for f in res.faces])
            if new_mesh.n_faces < 1:
                return jsonify({"error": f"{name} produced an empty mesh"}), 400
            _snap_obj(oid)
            o.mats = _transfer_mats(m, o.mats, new_mesh)
            o.mesh = new_mesh
            _bump(oid)
            out = _payload(only=oid)
            out["report"] = _json_safe(rep_) if isinstance(rep_, dict) else {}
            out["faces"] = {"before": int(m.n_faces), "after": int(new_mesh.n_faces)}
            return jsonify(out)
        ...
        if name == "halve":
            ...
            if o is None:
                return jsonify({"error": "no such object"}), 400
            import holographic.mesh_and_geometry.holographic_sdf as _S2
...
def render_engine():
    ...
    _init()
    g = request.args.get
    import holographic.rendering.holographic_render as _hr
    from holographic.mesh_and_geometry.holographic_mesh import Mesh as _Mesh
...
def import_glb():
    ...
    import holographic.io_and_interop.holographic_assetimport as _ai
            ...
            for gname, part in parts.items():
                ...
                if imp_mode in ("asis", "decimate", "retopo", "voxel", "rebake"):
                    ...
                    if imp_mode == "decimate" and subm.n_faces > imp_target:
                        import holographic.mesh_and_geometry.holographic_meshqem as _mq2
                        ...
                        import holographic.mesh_and_geometry.holographic_meshbridge as _mb
                    ...
                    if imp_reproject and uvsub is None and tex is not None and src_for_reproj[1] is not None                             and imp_mode not in ("asis", "rebake"):
                        ...
                        try:
                            uvsub, resid = _transfer_uv_resid(_mt, s_m, s_uv, subm.vertices)
                    ...
                    if any(len(f) != 3 for f in subm.faces):
                        ...
                        from holographic.mesh_and_geometry.holographic_mesh import Mesh as _Mesh2
                ...
                if tex is not None and uvsub is not None:
                    if imp_mode == "auto" and subm.n_faces > FACE_BUDGET:
                        ...
                        src_for_uv = subm
                        import holographic.mesh_and_geometry.holographic_meshqem as _mq
                        ...
                        try:
                            ...
                            raise RuntimeError(f"decimate_to refused: {type(_e).__name__}: {_e}")
                        subm, orep = _mt.mesh_orient(subm)
                        try:
                            luv, resid = _transfer_uv_resid(_mt, src_for_uv, uvsub, subm.vertices)
                            uv_route = "transfer_uv (barycentric" + \
                                f", median resid {resid:.5f})"
                        except Exception:
...
def _auto_uv(oid):
    ...
    from holographic.mesh_and_geometry.holographic_mesh import Mesh
    from holographic.mesh_and_geometry.holographic_meshuv import lscm, stable_uv, uv_angle_distortion
    ...
    if work.n_vertices > _UV_ISOMAP_MAX:
        ...
        if _safe_closed(work):
            from holographic.mesh_and_geometry.holographic_meshseam import shortest_seam, cut_seam
            a = 0
...
def uv_maps():
    ...
    with _LOCK:
        ...
        lines = ["# leStudio3d export (leCore) -- positions + auto-unwrapped vt"]
    ...
    return Response(buf.getvalue(), mimetype="application/zip",
                    headers={"Content-Disposition": "attachment; filename=lestudio3d_maps.zip"})
...
def export_glb():
    ...
    with _LOCK:
        ...
        if data is None:
            ...
            data = mesh_to_glb(m, base_colour=(float(alb[0]), float(alb[1]), float(alb[2]), 1.0),
                               generator="lestudio3d", texture=tex_arg)
    ...
    return Response(data, mimetype="model/gltf-binary",
                    headers={"Content-Disposition": "attachment; filename=lestudio3d.glb"})
...
def _bake_object(oid, res, with_ids):
    ...
    from holographic.mesh_and_geometry.holographic_meshbridge import flood_fill_sign, mesh_to_sdf
    from holographic.mesh_and_geometry.holographic_mesh import Mesh
    o = _S["objects"][oid]
    mesh = o.mesh
...
def _analytic_scene():
    ...
    import holographic.mesh_and_geometry.holographic_sdf as _sd
    scene = trees[0]
    for t in trees[1:]:
        scene = scene.union(t)
...
def _fsr_to(img, out_hw, sharpness=0.35):
    from holographic.rendering.holographic_fsr import lanczos_upscale
    from holographic.rendering.holographic_postfx import sharpen
...
def _cam_from_args(g):
    ...
    _S["camera"] = {"eye": eye, "target": target, "fov_deg": fov, "aspect": 1.0}   # published as lecore.camera
...
def render_progressive():
    ...
    with _LOCK:
        ...
        if st["converged"]:
            ...
            from holographic.rendering.holographic_raymarch import render_sdf as _rs
...
def render_preview():
    ...
    resp.headers["X-LeCore-Render"] = (f"fsr {out['Wi']}x{out['Hi']}->{out['W']}x{out['H']}; "
...
def photo():
    ...
    def _prep(hdr):
        ...
        try:
            from holographic.rendering.holographic_gemrender import clamp_fireflies
    ...
    import holographic.rendering.holographic_pathtrace as pt
    from holographic.rendering.holographic_postfx import denoise
...
def sculpt_stroke():
    ...
    with _LOCK:
        ...
        from holographic.mesh_and_geometry.holographic_sculpt import falloff
        s = o.sculpt
        s["dirty"] = True
...
def sculpt_exit():
    ...
    with _LOCK:
        ...
        if not o.sculpt.get("dirty"):
            ...
            return jsonify(_payload(only=oid))
        _push_snapshot(("grid", oid, o.sculpt["grid"].copy()))
        from holographic.mesh_and_geometry.holographic_meshqem import cluster_decimate
...
#
# WHY NOT the engine's "VSA program" machinery: that name refers to Vector-Symbolic-Architecture / hyperdimensional
...
def _tree_from_clauses(text):
    ...
    import holographic.mesh_and_geometry.holographic_sdf as S
    from holographic.io_and_interop.holographic_codecompose import _split_clauses, _match_form, _extract_params
    tree = None
...
def compose():
    ...
    from holographic.io_and_interop.holographic_codecompose import describe_to_kernel, ComposeError
    from holographic.io_and_interop.holographic_zigrun import as_numpy
    try:
    ...
    with _LOCK:
        ...
        try:
            stl = mesh_to_stl(o.mesh.vertices.tolist(), [list(f) for f in o.mesh.faces],
                              name=o.name.replace(" ", "_") or "lestudio3d")
        except Exception as e:
            return jsonify({"error": f"stl export failed: {e}"}), 400
        return Response(stl, mimetype="model/stl",
                        headers={"Content-Disposition": "attachment; filename=lestudio3d.stl"})
...
#
# leStudio3d holds ONE graph (this is a single-user demo). "Build" evaluates a node and lands the result in the
...
_NODE_PARAMS = {
    ...
    # ---- leStudio3d EXTENSIONS: fractals, fields (Shadertoy-style + curl noise), field-driven geometry,
...
def _augment_registry(reg):
    """Register leStudio3d's extension node types onto a fresh engine registry. Each DELEGATES to a real engine
...
def nodes_op():
    ...
    try:
        ...
        if act == "build":
            ...
            if muted:
                from holographic.scene_and_pipeline.holographic_nodegraph import NodeGraph as _NG
                dd = g.to_dict()
...
def export_obj():
    ...
    with _LOCK:
        ...
        if o is None:
            return jsonify({"error": "no such object"}), 400
        lines = ["# leStudio3d (leCore) export", "o " + (o.name.replace(" ", "_") or "object")]
        lines += ["v %.6f %.6f %.6f" % tuple(p) for p in o.mesh.vertices]
        lines += ["f " + " ".join(str(i + 1) for i in fc) for fc in o.mesh.faces]
        return Response("\n".join(lines) + "\n", mimetype="text/plain",
                        headers={"Content-Disposition": "attachment; filename=lestudio3d.obj"})


@bp.route("/api/docs")
def docs():
    """Search the engine's capability catalog with the engine's own ranker (m.find_capability): ?q= ranks by
    meaning, ?k= limits; empty q browses the catalog (m.browse_capabilities). Each hit carries name / does /
    aliases / example / module -- the PREPARE step of every feature ("does the engine do X?") answered in-app."""
    _init()
    mind = _mind()
    q = (request.args.get("q", "") or "").strip()
    k = int(np.clip(int(request.args.get("k", 12)), 1, 60))

    def card(c):
        return {"name": getattr(c, "name", str(c)), "does": str(getattr(c, "does", "") or "")[:240],
                "aliases": list(getattr(c, "aliases", []) or [])[:6], "example": getattr(c, "example", "") or "",
                "module": getattr(c, "module", "") or "", "theme": getattr(c, "module", "") or ""}
    if not q:
        try:
            cat = mind.browse_capabilities()
            items = cat if isinstance(cat, list) else list(cat)
        except Exception:
            items = []
        return jsonify({"available": True, "count": len(items), "results": [card(c) for c in items[:400]]})
    try:
        hits = mind.find_capability(q, k=k)
    except Exception as e:
        return jsonify({"available": False, "results": [], "note": f"find_capability refused: {e}"}), 400
    return jsonify({"available": True, "ranker": "engine.find_capability", "count": len(hits),
                    "results": [card(h) for h in hits]})
...
def section():
    ...
    resp.headers["X-LeCore-Section"] = (f"axis={'xyz'[axis]} offsets=" +
...
def photo_light():
    ...
    from holographic.rendering.holographic_shapefromshading import estimate_light
...
def photo_texture():
    ...
    from holographic.mesh_and_geometry.holographic_fitshape import fit_texture, _noise_glsl
    import holographic.sampling_and_signal.holographic_noise as hn
...
# =====================================================================================================
def _obj_shape(o):
    if o.sdf_tree is not None and hasattr(o.sdf_tree, "to_dsl"):
        dsl = o.sdf_tree.to_dsl()
...
def validate():
    ...
    with _LOCK:
        ...
        if o is None:
            return jsonify({"error": "no such object (pass 'object')"}), 400
        from holographic.mesh_and_geometry.holographic_meshtools import mesh_report
...
def repair_mesh():
    ...
    with _LOCK:
        ...
        from holographic.mesh_and_geometry.holographic_meshtools import mesh_repair, mesh_report
        _snap_obj(oid)
        try:
...
def upscale():
    ...
    try:
        from holographic.rendering.holographic_superres import easu_upscale
...
def data_analyze():
    ...
    import holographic.sampling_and_signal.holographic_demux as dx
...
_AGENT_HINTS = {
    ...
    "engine_status": "The engine's own engine_status() plus this app's features() gate: what is present, what is missing.",
...
# canonical image kind), and we write our objects as `lestudio3d.object` plus a `lecore.image` per texture
...
def _lews_resolve_assets(sections):
    """leStudio R67 hoists arrays shared between documents into `lecore.asset` sections and points at
    them from `meta.array_refs` -- put them back before anything reads `arrays[...]`. Mirrors leStudio's
    own _resolve_shared_arrays, which is the reader contract for its files."""
    pool = {}
    for sec in sections:
...
def _lews_upgrade(sec):
    """Lift a section to the schema this engine knows (register_kind_schema migrations); a section NEWER
    than the engine is carried read-only, which for reading a texture is fine."""
    try:
        from holographic.io_and_interop import holographic_lews as _L
        if hasattr(_L, "upgrade_section"):
            return _L.upgrade_section(sec)
    except Exception:
        pass
    return sec
...
def _lews_documents(container):
    ...
    out = []
    sections = _lews_resolve_assets(container.get("sections", []))
    for sec in sections:
        kind = sec.get("kind")
        try:
            if kind == "lestudio.document":
                sec = _lews_upgrade(sec)
                ...
                for r in recs:
                    ...
                    if key in sec["arrays"]:
                        ...
                        continue
                    # leStudio R67 saves JOURNAL-FIRST by default: a layer whose replay is provable carries
                    # NO pixels, only its replay base + the stroke journal (which needs leStudio's brushes to
                    # replay). We cannot replay; we use the base as the best available approximation and SAY
                    # SO, instead of composing zero layers and skipping the document silently (what 1.5.0 did).
                    base = "replaybase_%s" % r.get("id")
                    if base in sec["arrays"]:
                        layers[r.get("id")] = np.asarray(sec["arrays"][base], float)
                    journal_only.append(str(r.get("id")))
                if not layers:
                    continue
                ...
                out.append({"id": str(sec.get("id") or meta.get("id") or len(out)),
                            ...
                            "layers": len(layers), "journal_only": journal_only,
                            "rgb": np.clip(img[:, :, :3], 0.0, 1.0)})
            elif kind == "lecore.image" and not str(sec.get("id", "")).startswith("lestudio3d:"):
...
def workspace_inspect():
    ...
    return jsonify({"app": (cont.get("meta") or {}).get("app"),
                    ...
                    "textures": [{"id": d["id"], "name": d["name"], "w": d["w"], "h": d["h"],
                                  "layers": d["layers"], "journal_only": d.get("journal_only", [])}
                                 for d in docs],
                    "note": ("some layers were saved journal-first by leStudio (no pixels); their replay "
                             "base is used as an approximation -- bake pixels in leStudio for an exact texture"
                             if any(d.get("journal_only") for d in docs) else None)})
...
def workspace_import():
    ...
    cont = None
    if raw[:1] == b"{":
        # ADOPTED (1.6.0): {"from":"workspace"} imports from the LIVE .lews directory -- what leStudio
        # published with Workspace.put this second -- no file round-trip; also lecore.mesh sections
        # (a mesh any app put) become objects.
        try:
            j = json.loads(raw.decode("utf-8"))
        except Exception:
            j = {}
        if j.get("from") == "workspace":
            ws = _workspace()
            if ws is None:
                return jsonify({"error": "no live workspace: set LESTUDIO3D_WORKSPACE=<dir>"}), 400
            cont = {"sections": _ws_sections(ws), "meta": {}}
    if cont is None:
        ws = _workspace()
        if ws is not None:
            # a file INTO the live directory: the engine's import carries every section verbatim and journals one
            # line saying the content came from a file, so a late-joining app can tell "opened" from "edited"
            import tempfile
            from holographic.io_and_interop.holographic_lews import Workspace as _W
            fd, tmp = tempfile.mkstemp(suffix=".lews"); os.write(fd, raw); os.close(fd)
            try:
                global _WS
                _WS = _W.from_file(tmp, _ws_root(), app="lestudio3d", app_version=_app_version())
                _CANON_SHA.clear()
            except Exception as e:
                return jsonify({"error": f"Workspace.from_file refused: {type(e).__name__}: {e}"}), 400
            finally:
                os.unlink(tmp)
            cont = {"sections": _ws_sections(_WS), "meta": {}}
        else:
            try:
                cont = load_container(raw)
            except Exception as e:
                return jsonify({"error": f"not a leCore workspace container: {e}"}), 400
    docs = _lews_documents(cont)
    meshes = _lews_meshes(cont)
    sdfs = _lews_sdfs(cont)
    cam = _lews_camera(cont)
    if not docs and not meshes and not sdfs and cam is None:
        return jsonify({"error": "nothing to import: no lecore.image / lestudio.document / lecore.mesh / "
                                 "lecore.sdf / lecore.camera sections from ANOTHER app (our own scene is loaded "
                                 "with POST /api/scene/load {\"from\": \"workspace\"})"}), 400
    with _LOCK:
        ...
        _S["ws_foreign"] = [s for s in cont.get("sections", [])
                            if s.get("kind") not in ("lestudio3d.object",)]
        _S["ws_meta"] = cont.get("meta") or {}
        store = _S.setdefault("ws_textures", {})
        for d in docs:
            store[d["id"]] = {"name": d["name"], "rgb": d["rgb"]}
        imported = []
        for ms in meshes:                                   # canonical geometry from ANY app -> objects
            try:
                from holographic.mesh_and_geometry.holographic_mesh import Mesh as _M
                mesh = _M(ms["verts"], [tuple(int(v) for v in f) for f in ms["faces"]])
                nid = _add_object(ms["name"], mesh, [ms.get("material") or _default_mat_name()] * mesh.n_faces)
                if ms.get("uv") is not None and len(ms["uv"]) == mesh.n_vertices:
                    _S["render_assets"][nid] = {"uv": np.asarray(ms["uv"], float)}
                imported.append(nid)
            except Exception:
                continue
        for ss in sdfs:                                     # exact geometry from any app: meshed, tree KEPT
            try:
                mesh = _mesh_sdf_tree(ss["tree"], res=56)
                nid = _add_object(ss["name"], mesh, [ss.get("material") or _default_mat_name()] * mesh.n_faces,
                                  sdf_tree=ss["tree"])
                imported.append(nid)
            except Exception:
                continue
        if cam is not None:
            _S["camera"] = cam                              # the render camera another app framed
        if imported or cam is not None:
            _S["rev"] += 1
        target = str(request.args.get("object", ""))
        applied = None
        if docs and target and target in _S["objects"]:
            o = _S["objects"][target]
            uv = None
            ...
            _S["render_assets"][target] = {"uv": uv, "tex": docs[0]["rgb"]}   # noqa
            _S["rev"] += 1
        ...
        out["workspace"] = {"textures": [{"id": d["id"], "name": d["name"], "w": d["w"], "h": d["h"]}
                                         for d in docs],
                            "applied_to": applied, "imported_meshes": imported, "camera": cam,
...
def workspace_export():
    """Download the workspace as a .lews file. With a live workspace open this is `Workspace.export_bytes()`
    -- the shared directory itself (every app's sections, assets, presets), after our scene is published into
    it. Without one, a container of our canonical sections plus every foreign section carried from an import."""
    _init()
    from holographic.io_and_interop.holographic_container import save_container, image_section
    ws = _workspace()
    if ws is not None:
        with _LOCK:
            _publish_canonical(ws)
        resp = Response(ws.export_bytes(), mimetype="application/octet-stream")
    ...
    with _LOCK:
        ...
        for oid, o in _S["objects"].items():
            ...
            a = _S.get("render_assets", {}).get(oid)
            uv = np.asarray(a["uv"], np.float32) if a and "uv" in a and len(a["uv"]) == m.n_vertices else None
            sections.append(_mind().lews_mesh_section(np.asarray(m.vertices, np.float32), F,
                                                      sid=f"lestudio3d:mesh:{oid}", name=o.name, uv=uv))
            if o.sdf_tree is not None:
                try:
                    sections.append(_mind().lews_sdf_section(o.sdf_tree.to_dsl(), sid=f"lestudio3d:sdf:{oid}", name=o.name))
                except Exception:
                    pass
            if a and "tex" in a:
        ...
        meta.setdefault("app", "lestudio3d")              # our own files identify as ours
...
def engine_status():
    """What Help > Engine status shows: the engine's own engine_status() (version, faculty count, which optional
    extras import, GPU report, determinism policy, CPU budget) plus this app's features() gate from the mount.
    No client-side list of what to check -- APP_FOUNDATION §1: it rots silently."""
    st = _json_safe(_mind().engine_status())
    st["features"] = dict(_FOUNDATION.get("have") or {})
    st["missing"] = [k for k, v in st["features"].items() if not v]
    st["workspace"] = _ws_root()
    return jsonify(st)
...
def _ws_root():
    return os.environ.get("LESTUDIO3D_WORKSPACE")


def _workspace():
    """The live .lews workspace, if LESTUDIO3D_WORKSPACE names one (docs/APP_FOUNDATION.md §3).
    ...
    if _WS is None:
        ...
        if root:
            try:
                _WS = _mind().lews_open(root, app="lestudio3d")
            except Exception:
                _WS = False
        else:
            _WS = False
...
def workspace_status():
    ...
    ws = _workspace()
    if ws is None:
        return jsonify({"open": False, "hint": "set LESTUDIO3D_WORKSPACE=<dir> to share a live .lews"})
    since = request.args.get("since")
    out = {"open": True, "root": _ws_root()}
    for k in ("describe", "roster"):
        try: out[k] = _json_safe(getattr(ws, k)())
        except Exception as e: out[k] = f"{type(e).__name__}: {e}"
    try:
        out["presence"] = _json_safe(_mind().lews_presence(_ws_root()))
        out["kinds"] = sorted({s.get("kind") for s in _ws_sections(ws)})
        out["assets"] = len(ws.asset_refs()) if hasattr(ws, "asset_refs") else None
    except Exception:
        pass
...
def scene_save():
        ...
        for oid, o in _S["objects"].items():
            ...
            if lay:                                            # ocean wave-spec / creature walk-spec survive load
                ...
                if safe:
                    entry["layers"] = safe
            par = _PARENT.get(oid)
            if par in _S["objects"]:                            # ids are stable across save/load: store the id
                entry["parent"] = str(par)
            h = _HIST.get(oid)
    ...
    out = {"version": 1, "objects": objs, "custom_materials": mats,
           "note": "leStudio3d scene. Objects are stored as meshes (analytic primitive trees are "
    ...
    with _LOCK:
        ...
        if ws is not None:
            ...
            try:
                from holographic.io_and_interop import holographic_lews as _L
                out["workspace_rev"] = ws.put({"id": "lestudio3d.scene", "kind": "lestudio3d.scene",
                                               "meta": {"scene": out}})
                # ADOPTED (1.6.0): the CANONICAL kinds beside our private blob -- lecore.mesh per object,
                # lecore.image per texture, lecore.material per custom material, one lecore.scene of
                # bindings -- so a painter, an agent shell, or the next modeller reads our scene without
                # knowing leStudio3d's file. Unchanged sections are skipped by section_hash (a put
                # rewrites the container and journals a line; no-op churn would spam every feed).
                out["workspace_rev"] = _publish_canonical(ws) or out["workspace_rev"]
            except Exception as e:
...
def scene_load():
    ...
    if not d or d.get("from") == "workspace":
        ...
        ws = _workspace()
        sec = ws.get("lestudio3d.scene") if ws is not None else None
        if sec and isinstance(sec.get("meta"), dict) and "scene" in sec["meta"]:
            d = sec["meta"]["scene"]
        elif ws is not None:
            return jsonify({"error": "workspace has no lestudio3d.scene section yet"}), 404
    if "objects" not in d:
    ...
    with _LOCK:
        ...
        pending_parents = []                                  # (child id, parent id)
        for ob in d["objects"]:
            ...
            nid = _add_object(ob.get("name", "Object"), mesh, mats, oid=ob.get("id"))   # ids survive a load
            lay = ob.get("layers")
            if lay:                                            # restore ocean/walk specs so animation works post-load
                _S["objects"][nid].layers = dict(lay)
            if ob.get("parent"):
                pending_parents.append((nid, str(ob["parent"])))
            hs = ob.get("history")
        ...
        for child_id, pid in pending_parents:                 # A2-3: re-link the hierarchy by id
            if pid in _S["objects"] and pid != child_id:
                _PARENT[child_id] = pid
        # FULL-FIDELITY RESTORE: node graph + layout, render assets (textures), units
        try:
            nd = d.get("nodes")
            if nd and nd.get("graph"):
                from holographic.scene_and_pipeline.holographic_nodegraph import NodeGraph as _NG, default_registry as _dreg
        ...
        try:
            _S["render_assets"] = {}
            for noid, e in (d.get("render_assets") or {}).items():
                noid = str(noid)
                if noid not in _S["objects"]:
                    continue
...
def sweep():
    ...
    import holographic.mesh_and_geometry.holographic_curves as cv
    from holographic.mesh_and_geometry.holographic_mesh import Mesh
    with _LOCK:
        try:
...
def sketch_solve():
    ...
    if d.get("extrude"):                                        # hand the solved polygon to the profile extruder
        with _LOCK:
            try:
                import holographic.mesh_and_geometry.holographic_sdf2d as s2
                from holographic.mesh_and_geometry.holographic_meshbridge import marching_tetrahedra_vec
                from holographic.mesh_and_geometry.holographic_meshqem import cluster_decimate
                from holographic.mesh_and_geometry.holographic_mesh import Mesh
...
def bounding_boxes():
    ...
    with _LOCK:
        ...
        import holographic.mesh_and_geometry.holographic_fitshape as _fs
...
def generate():
    ...
    with _LOCK:
        ...
        if kind == "landscape":
            from holographic.mesh_and_geometry.holographic_terrain import Terrain, terrain_to_mesh
...
def erode_terrain():
    ...
    with _LOCK:
        ...
        H0 = H.copy()
        import holographic.mesh_and_geometry.holographic_terrain as _tr
...
def env_backdrop():
    ...
    # ADOPTED (1.6.0): the engine's OWN lights as dome sources, rasterised into the same equirect image the
    # renderer already samples: {"studio":"classic|soft|dramatic"} (three-point rig as an environment,
    # m.studio_sky), {"sky":{"hour":19,"clouds":[["cirrus",0.5]]}} (parametric sky, m.sky_model),
    # {"exr":"<path>"} (OpenEXR via m.load_exr, opt-in `pip install OpenEXR`).
    src = _env_from_engine(d)
    if src is not None:
        img, meta = src
        with _LOCK:
            _S["env_img"] = img; _S["env_meta"] = meta
        prev = np.clip(img / max(float(np.percentile(img, 99.5)), 1e-6), 0, 1)
        return jsonify({"active": True, **meta,
                        "preview": _png_b64((prev ** (1 / 2.2) * 255).astype(np.uint8))})
...
def select_geodesic():
    ...
    with _LOCK:
        ...
        import holographic.mesh_and_geometry.holographic_meshgeodesic as mg
        try:
...
def fitprims():
    ...
    with _LOCK:
        ...
        if o is None:
            return jsonify({"error": "no such object"}), 400
        import holographic.mesh_and_geometry.holographic_primfit as pf
...
# =====================================================================================================
# 1.6.0 -- THE FOUNDATION, MOUNTED HERE (docs/APP_FOUNDATION.md), and the engine's Scene document as
# the agent's view of this scene.
#
# WHY HERE and not app.py: 1.5.0 built one UnifiedMind in app.py and a second one lazily in backend
# (rule 1 broken: "one mind"), and the test harnesses -- which boot the blueprint without app.py -- lost
# the agent surface entirely (render_api_test failed on /api/agent/tools). One mount, one mind, called by
# the launcher AND the harnesses.
# =====================================================================================================

def _json_safe(x):
    """numpy -> plain JSON, recursively; anything else -> its repr. For engine reports in replies."""
    if isinstance(x, dict):
        return {str(k): _json_safe(v) for k, v in x.items()}
    if isinstance(x, (list, tuple)):
        return [_json_safe(v) for v in x]
    if isinstance(x, np.ndarray):
        return x.tolist() if x.size <= 4096 else {"shape": list(x.shape), "dtype": str(x.dtype)}
...
def _png_b64(arr_u8):
    """uint8 (H,W,3) -> base64 PNG (no data: prefix; app.js adds it, as for the procedural presets). (1.5.0 referenced this name behind a globals() check; it never existed.)"""
    from PIL import Image
    buf = io.BytesIO(); Image.fromarray(np.asarray(arr_u8, np.uint8)).save(buf, "PNG")
    return base64.b64encode(buf.getvalue()).decode()          # raw b64: the client prepends the data: prefix


def _equirect_dirs(H, W):
...
def _env_from_engine(d):
    """{"studio":...} | {"sky":{...}} | {"exr":...} -> (equirect float32 (H,W,3), meta), or None."""
    H, W = (256, 512) if str(d.get("quality", "dome")) == "high" else (128, 256)
    mind = _mind()
    if d.get("exr"):
        img = np.asarray(mind.load_exr(str(d["exr"]), exposure=float(d.get("exposure", 1.0))), np.float32)
        return img, {"source": "exr", "shape": list(img.shape), "max_radiance": float(img.max())}
    if d.get("studio"):
        preset = str(d["studio"])
        sky = mind.studio_sky(preset, gain=float(d.get("gain", 1.0)))
        rad = np.asarray(sky(_equirect_dirs(H, W)), np.float32).reshape(H, W, -1)[:, :, :3]
        meta = {"source": "studio", "preset": preset, "shape": [H, W, 3], "max_radiance": float(rad.max())}
        try:
            meta["ratios"] = _json_safe(mind.rig_ratios())
        except Exception:
            pass
        return rad, meta
    if d.get("sky") is not None:
        sp = d["sky"] if isinstance(d["sky"], dict) else {}
        clouds = [(str(c[0]), float(c[1])) for c in (sp.get("clouds") or []) if len(c) >= 2]
        kw = {"hour": float(sp.get("hour", 12.0)), "clouds": clouds}
        if "seed" in sp:
            kw["seed"] = int(sp["seed"])
        if sp.get("moon"):
            kw["moon"] = True
        sky = mind.sky_model(**kw)
        rad = np.asarray(sky(_equirect_dirs(H, W)), np.float32).reshape(H, W, -1)[:, :, :3]
        return rad, {"source": "sky", "hour": kw["hour"], "clouds": clouds, "shape": [H, W, 3],
                     "max_radiance": float(rad.max())}
    return None


# ---------------------------------------------------------------- the live workspace, canonical kinds
def _ws_sections(ws):
    try:
        return list(ws.sections(upgrade=True))
    except TypeError:
        return list(ws.sections())


def _lews_bindings(container):
    """lecore.scene sections from OTHER apps -> {mesh/sdf section id: {material name, texture id}}."""
    out = {}
    for sec in container.get("sections", []):
        if sec.get("kind") != "lecore.scene" or str(sec.get("id", "")).startswith("lestudio3d:"):
            continue
        for b in ((sec.get("meta") or {}).get("objects") or []):
            if b.get("mesh"):                               # LC-8: scene bindings carry mesh, not sdf/name
                out[str(b["mesh"])] = b
    return out


def _lews_material_name(container, mat_sid):
    """A lecore.material section id -> the matlib name this app can render: overrides.matlib, then the
    optics library name, then the section's own name if the library knows it."""
    for sec in container.get("sections", []):
        if sec.get("kind") == "lecore.material" and str(sec.get("id")) == str(mat_sid):
            meta = sec.get("meta") or {}
            for cand in ((meta.get("overrides") or {}).get("matlib"), meta.get("library"), meta.get("name")):
                if cand:
                    try:
                        _mat(str(cand)); return str(cand)
                    except Exception:
                        continue
    return None


def _lews_sdfs(container):
    """Every canonical `lecore.sdf` another app put -> [{name, tree, material}] via the engine's parse_dsl."""
    out = []
    bind = _lews_bindings(container)
    from holographic.mesh_and_geometry.holographic_sdf import parse_dsl
    for sec in container.get("sections", []):
        if sec.get("kind") != "lecore.sdf":
            continue
        sid = str(sec.get("id") or "")
        if sid.startswith("lestudio3d:"):
            continue
        dsl = (sec.get("meta") or {}).get("dsl") or (sec.get("meta") or {}).get("text")
        if not dsl:
            continue
        try:
            tree = parse_dsl(str(dsl))
        except Exception:
            continue
        b = bind.get(sid, {})
        out.append({"id": sid, "name": str(b.get("name") or (sec.get("meta") or {}).get("name") or sid),
                    "tree": tree, "material": _lews_material_name(container, b.get("material")) if b.get("material") else None})
    return out


def _lews_camera(container):
    """The newest `lecore.camera` another app put -> {eye, target, fov_deg, aspect} or None."""
    cam = None
    for sec in container.get("sections", []):
        if sec.get("kind") == "lecore.camera" and not str(sec.get("id", "")).startswith("lestudio3d:"):
            m = sec.get("meta") or {}
            if m.get("eye") is not None and m.get("target") is not None:
                cam = {"eye": [float(x) for x in m["eye"]], "target": [float(x) for x in m["target"]],
                       "fov_deg": float(m.get("fov_deg", 40.0)), "aspect": float(m.get("aspect", 1.0))}
    return cam


def _lews_meshes(container):
    """Every canonical `lecore.mesh` section another app (or agent) put -> [{name, verts, faces, uv,
    material}]. Our own meshes are skipped by id prefix so a re-import does not duplicate the scene."""
    out = []
    bind = _lews_bindings(container)
    for sec in _lews_resolve_assets(container.get("sections", [])):
        if sec.get("kind") != "lecore.mesh":
            continue
        sid = str(sec.get("id") or "")
        if sid.startswith("lestudio3d:"):
            continue
        meta, arrays = sec.get("meta") or {}, sec.get("arrays") or {}
        V, F = arrays.get("verts", arrays.get("vertices")), arrays.get("faces")
        if V is None or F is None:
            continue
        b = bind.get(sid, {})
        mat = _lews_material_name(container, b.get("material")) if b.get("material") else meta.get("material")
        out.append({"id": sid, "name": str(b.get("name") or meta.get("name") or sid or "mesh"),
                    "verts": np.asarray(V, float).reshape(-1, 3), "faces": np.asarray(F, int).reshape(-1, 3),
                    "uv": arrays.get("uv"), "material": mat, "texture": b.get("texture")})
    return out


_CANON_SHA = {}                                             # section id -> hash of what we last put


def _publish_canonical(ws):
    """Put the canonical view of the scene into the live workspace; return the last rev written or None."""
    from holographic.io_and_interop import holographic_lews as _L
    ...
    def put(sec):
        nonlocal rev
        sha = _L.section_hash(sec)
        want.add(sec["id"])
        if _CANON_SHA.get(sec["id"]) == sha:
            return
        rev = ws.put(sec); _CANON_SHA[sec["id"]] = sha

    bindings = []
    for oid, o in _S["objects"].items():
        m = o.mesh
        F = np.array([list(f)[:3] for f in m.triangulate()], dtype=np.int32)
        a = _S.get("render_assets", {}).get(oid) or {}
        uv = np.asarray(a["uv"], np.float32) if "uv" in a and len(a["uv"]) == m.n_vertices else None
        mid = f"lestudio3d:mesh:{oid}"
        put(mind.lews_mesh_section(np.asarray(m.vertices, np.float32), F, sid=mid, name=o.name, uv=uv))
        if o.sdf_tree is not None:                          # exact geometry beside the mesh: the tree IS the object
            try:
                put(mind.lews_sdf_section(o.sdf_tree.to_dsl(), sid=f"lestudio3d:sdf:{oid}", name=o.name))
            except Exception:
                pass
        h = _HIST.get(oid)
        if h and any(h["branches"].values()):               # the op journal: what was done, replayable, not pixels
            try:
                ops = [dict(op, branch=br) for br, lst in h["branches"].items() for op in lst]
                put(mind.lews_journal_section(mid, ops, sid=f"lestudio3d:journal:{oid}", name=f"{o.name} edits"))
            except Exception:
                pass
        mats = sorted({str(x) for x in np.asarray(o.mats).ravel()})
        matid = f"lestudio3d:mat:{mats[0]}" if mats else None
        b = {"id": str(oid), "name": o.name, "mesh": mid}
        # (the sdf twin is lestudio3d:sdf:<oid>; the engine's scene_section drops an "sdf" binding -- LC-8)
        if matid:
            b["material"] = matid
        if "tex" in a:
            tid = f"lestudio3d:tex:{oid}"
            sec = image_section(np.clip(np.asarray(a["tex"], float), 0, 1), colour_space="srgb", dpi=72.0,
                                name=f"{o.name} texture")
            sec["id"] = tid
            put(sec); b["texture"] = tid
        bindings.append(b)
    for name in sorted({str(x) for o in _S["objects"].values() for x in np.asarray(o.mats).ravel()}):
        try:
            mat = _mat(name)
        except Exception:
            continue
        ov = {"matlib": name, "base_color": [round(float(c), 4) for c in np.asarray(mat.base_color)[:3]],
              "metallic": round(float(getattr(mat, "metallic", 0.0)), 4),
              "roughness": round(float(getattr(mat, "roughness", 0.5)), 4)}
        for k in ("transmission", "ior"):
            if hasattr(mat, k):
                ov[k] = round(float(getattr(mat, k)), 4)
        lib = None
        try:
            from holographic.materials_and_texture.holographic_matlib import _IOR, _ABSORB
            lib = name if (name in _IOR or name in _ABSORB) else None
        except Exception:
            pass
        put(mind.lews_material_section(name, sid=f"lestudio3d:mat:{name}", library=lib, overrides=ov))
    put(mind.lews_scene_section(bindings, sid="lestudio3d:scene", name="leStudio3d scene"))
    cam = _S.get("camera")
    if cam:                                                 # the render camera, so another app frames the same view
        put(mind.lews_camera_section(tuple(cam["eye"]), tuple(cam["target"]), fov_deg=float(cam.get("fov_deg", 40.0)),
                                     aspect=float(cam.get("aspect", 1.0)), sid="lestudio3d:camera"))
    if _S.get("env_img") is not None:                       # the environment: one content-addressed asset,
        try:                                                # referenced from a private section so GC keeps it
            key = ws.put_asset(np.asarray(_S["env_img"], np.float32), name="environment")
            meta = {k: v for k, v in (_S.get("env_meta") or {}).items() if k != "asset"}
            meta["env_asset"] = key
            put({"kind": "lestudio3d.environment", "id": "lestudio3d:env", "meta": _json_safe(meta), "arrays": {}})
        except Exception:
            pass
    # sections we published for objects that no longer exist
    for sid in [k for k in _CANON_SHA if k not in want and k.startswith("lestudio3d:")]:
        try:
            rev = ws.delete(sid)
        except Exception:
            pass
        _CANON_SHA.pop(sid, None)
    return rev


# ---------------------------------------------------------------- presets (lecore.preset)
_PRESETS = {}                                               # in-memory when no workspace is open


@bp.route("/api/presets", methods=["GET", "POST", "DELETE"])
def presets():
    """`lecore.preset` sections: a material, a render setup, an environment -- JSON recipes any leCore app
    can list and apply. GET ?target=lestudio3d.render lists; POST {name, target, params, tags} saves (into
    the live workspace when one is open, else this session); DELETE ?id= removes. A render preset saved here
    is at least listable in leStudio; a brush preset saved there is listable here."""
    _init()
    ws = _workspace()
    if request.method == "GET":
        target = request.args.get("target")
        secs = [s for s in _ws_sections(ws) if s.get("kind") == "lecore.preset"] if ws is not None \
            else list(_PRESETS.values())
        out = []
        for s in secs:
            meta = s.get("meta") or {}
            if target and meta.get("target") != target:
                continue
            out.append({"id": s.get("id"), "name": meta.get("name"), "target": meta.get("target"),
                        "params": meta.get("params"), "tags": meta.get("tags"), "app": meta.get("app"),
                        "author": meta.get("author")})
        return jsonify({"presets": out, "workspace": ws is not None})
    if request.method == "DELETE":
        pid = str(request.args.get("id", ""))
        if ws is not None:
            try:
                ws.delete(pid)
            except Exception as e:
    ...
    d = request.get_json(force=True) or {}
    name, target, params = str(d.get("name", "")).strip(), str(d.get("target", "")).strip(), d.get("params")
    if not name or not target or not isinstance(params, dict):
        return jsonify({"error": "need name, target (e.g. lestudio3d.render) and a params object"}), 400
    try:
        json.dumps(params)
    except (TypeError, ValueError):
        return jsonify({"error": "params must be JSON"}), 400
    sid = str(d.get("id") or f"preset:{target}:{name}")
    sec = _mind().lews_preset_section(name, target, params, sid=sid, tags=tuple(d.get("tags") or ()),
                                      author=str(request.headers.get("X-User", "")))
    if ws is not None:
        rev = ws.put(sec)
        return jsonify({"id": sid, "workspace_rev": rev})
    _PRESETS[sid] = sec
    return jsonify({"id": sid, "workspace_rev": None})


# ---------------------------------------------------------------- per-user memory (app_substrate)
_SUBSTRATES = {}


def _substrate(user):
    key = str(user or "default")
    sub = _SUBSTRATES.get(key)
    if sub is None:
        sub = _SUBSTRATES[key] = _mind().app_substrate("lestudio3d", user=key)
    return sub


@bp.route("/api/memory", methods=["POST"])
def memory():
    """Per-user memory through the engine's app_substrate (APP_FOUNDATION §6): a physically separate
    partition per (app, X-User). {action: remember, question, answer} | {action: recall, question} |
    {action: observe, event} | {action: suggest, context} | {action: habits} | {action: forget, question}.
    The command palette uses `suggest` to rank what this user does next."""
    d = request.get_json(force=True) or {}
    act = str(d.get("action", "recall"))
    try:
        sub = _substrate(request.headers.get("X-User") or d.get("user"))
    except Exception as e:
        return jsonify({"error": f"substrate unavailable: {type(e).__name__}: {e}"}), 503
    try:
        if act == "remember":
            r = sub.remember(str(d.get("question", "")), str(d.get("answer", "")))
        elif act == "recall":
            r = sub.recall(str(d.get("question", "")))
        elif act == "observe":
            r = sub.observe(d.get("event"))
        elif act == "suggest":
            r = sub.suggest(d.get("context"))
        elif act == "habits":
            r = sub.habits()
        elif act == "forget":
            r = sub.forget(str(d.get("question", "")))
        elif act == "stats":
            r = sub.stats()
        else:
            return jsonify({"error": "unknown action"}), 400
    except Exception as e:
        return jsonify({"error": f"{act} refused: {type(e).__name__}: {e}"}), 400
    return jsonify({"action": act, "user": sub.user, "result": _json_safe(r)})


# ---------------------------------------------------------------- the engine's Scene document
# The catalog says it plainly: "leCore had TWO scene systems that could not talk -- build_scene's
# SemanticScene and the Scene document (handles/undo, where every parity faculty landed)". leStudio3d's
# /api/semantic rides the first. This bridge puts THIS scene into the second, where describe_to_scene,
# scene_info, place, scene_set_texture, render_preview and render_animation live -- the doors an agent
# uses in Blender-MCP fashion. scene_add refuses a Mesh but takes any object with .eval, which is exactly
# what our per-object fields are (exact tree where possible, baked grid otherwise).
_DOC = {"scene": None, "rev": -1, "handles": {}}


def _scene_doc(res=None):
    """This scene as an engine Scene document, rebuilt when the scene revision moves."""
    if _DOC["scene"] is not None and _DOC["rev"] == _S["rev"]:
        return _DOC["scene"]
    mind = _mind()
    s = mind.new_scene()
    handles = {}
    res = int(res or _PHOTO_RES)
    for oid, o in _S["objects"].items():
        field = _exact_field_for(o)
        if field is None:
            field = _bake_object(oid, res, False)
        mats = sorted({str(x) for x in np.asarray(o.mats).ravel()})
        mat = None
        try:
            mat = _mat(mats[0]) if mats else None
        except Exception:
            mat = None
        h = mind.scene_add(s, name=o.name, geometry=field, material=mat, tags={"app": "lestudio3d", "oid": str(oid)})
        handles[h] = oid
    _DOC.update(scene=s, rev=_S["rev"], handles=handles)
    return s


def _doc_camera(d):
    """Camera from the request (eye/target/fov) or auto-fit to the scene's vertices (m.fit_camera)."""
    mind = _mind()
    W, H = int(np.clip(int(d.get("w", 240)), 32, 1024)), int(np.clip(int(d.get("h", 180)), 32, 1024))
    if d.get("eye") is not None:
        return mind.camera(eye=tuple(map(float, d["eye"])), target=tuple(map(float, d.get("target", (0, 0, 0)))),
                           fov_deg=float(d.get("fov", 50.0)), aspect=W / H), W, H
    sc = _S.get("camera")
    if sc and not d.get("fit"):                                # the last render / imported camera, if any
        return mind.camera(eye=tuple(sc["eye"]), target=tuple(sc["target"]), fov_deg=float(sc.get("fov_deg", 45)),
                           aspect=W / H), W, H
    V = [np.asarray(o.mesh.vertices, float) for o in _S["objects"].values()]
    if not V:
        return mind.camera(eye=(2.5, 1.8, 3.0), target=(0, 0, 0), aspect=W / H), W, H
    from holographic.mesh_and_geometry.holographic_mesh import Mesh
    allv = np.concatenate(V, 0)
    fit = mind.fit_camera(Mesh(allv, [(0, 1, 2)]) if len(allv) >= 3 else Mesh(np.vstack([allv] * 3), [(0, 1, 2)]),
                          width=W, height=H)
    return mind.camera(eye=tuple(fit["eye"]), target=tuple(fit["target"]), up=tuple(fit.get("up", (0, 1, 0))),
                       fov_deg=float(fit.get("fov_deg", 50.0)), aspect=W / H), W, H


@bp.route("/api/scene/doc")
def scene_doc_info():
    """What is in my scene, as the ENGINE reads it (m.scene_info): handles, names, geometry kind, material,
    the pre-flight problems list. An agent reads this first; it maps handle -> leStudio3d object id."""
    _init()
    with _LOCK:
        s = _scene_doc()
        info = _json_safe(_mind().scene_info(s))
        info["handles"] = dict(_DOC["handles"])
        try:
            info["ref"] = _scene_doc_ref()                  # drive it with any faculty via POST /engine/invoke
        except Exception:
            pass
        return jsonify(info)


@bp.route("/api/camera/fit")
def camera_fit():
    """m.fit_camera over every vertex in the scene: the camera dict that frames the subject exactly,
    aspect-aware, for w x h. The Render view's 'Frame all' uses this instead of a bbox guess."""
    _init()
    with _LOCK:
        d = {"w": request.args.get("w", 640), "h": request.args.get("h", 360), "fit": True}
        cam, W, H = _doc_camera(d)
    return jsonify({"eye": [float(x) for x in cam.eye], "target": [float(x) for x in cam.target],
                    "up": [float(x) for x in getattr(cam, "up", (0, 1, 0))], "fov_deg": float(getattr(cam, "fov_deg", 50.0)),
                    "w": W, "h": H})


@bp.route("/api/scene_preview")
def scene_doc_preview():
    """m.render_preview of the Scene document (mounted at /api/scene_preview, NOT /api/scene/preview: the
    engine's agent surface classifies image routes by the FIRST path segment, so a preview under scene/ would
    have marked scene/save and scene/load as images too -- LC-4 in LECORE_CORE_BACKLOG.md): the rough look in a fraction of a photo's cost (the engine
    measured 12x), for the see -> fix loop. ?w=&h=&eye=x,y,z&target=x,y,z&quality=draft|good. Returns a PNG."""
    _init()
    with _LOCK:
        d = {k: request.args.get(k) for k in ("w", "h", "quality") if request.args.get(k)}
        for k in ("eye", "target"):
            if request.args.get(k):
                d[k] = [float(x) for x in request.args.get(k).split(",")]
        s = _scene_doc()
        cam, W, H = _doc_camera(d)
        kw = {}
        if _S.get("env_img") is not None:
            env = np.asarray(_S["env_img"], np.float32)

            def sky(D):
                D = np.atleast_2d(np.asarray(D, float))
                el = np.arcsin(np.clip(D[:, 1], -1, 1)); az = np.arctan2(D[:, 2], D[:, 0]) % (2 * np.pi)
                y = np.clip(((1 - el / (np.pi / 2)) * 0.5 * (env.shape[0] - 1)).astype(int), 0, env.shape[0] - 1)
                x = np.clip((az / (2 * np.pi) * env.shape[1]).astype(int), 0, env.shape[1] - 1)
                return env[y, x, :3]
            kw["sky"] = sky
        img = _mind().render_preview(s, cam, W, H, quality=str(d.get("quality", "draft")), **kw)
    resp = Response(_png_bytes(np.asarray(img, float)), mimetype="image/png")
    resp.headers["X-Scene-Objects"] = str(len(_DOC["handles"]))
    return resp


@bp.route("/api/scene/describe", methods=["POST"])
def scene_describe():
    """Words -> objects. {text: "a red cube and a green sphere"} goes through the engine's describe_to_scene
    (the Scene document, not the SemanticScene grammar): every handled object comes back as a leStudio3d
    object -- meshed from its exact SDF (kept as the analytic tree, so it still exports as a shader), placed
    by its transform, its described colour mapped to the nearest library material (reported). `unknown`
    and `suggestions` are the engine's own, passed through so an agent can rephrase."""
    _init()
    d = request.get_json(force=True) or {}
    text = str(d.get("text", "")).strip()
    if not text:
    ...
    mind = _mind()
    try:
        r = mind.describe_to_scene(text)
    except Exception as e:
        return jsonify({"error": f"describe_to_scene refused: {type(e).__name__}: {e}"}), 400
    s = r.get("scene")
    made, placed = [], []
    with _LOCK:
        _snap_scene()
        for h, obj in (s.objects.items() if s is not None else []):
            geo = obj.geometry
            if geo is None or not hasattr(geo, "eval"):
                continue
            T = np.asarray(obj.transform, float) if obj.transform is not None else np.eye(4)
            tree = geo
            try:
                mesh = _mesh_sdf_tree(tree, res=int(np.clip(int(d.get("res", 56)), 24, 96)))
            except Exception:
                continue
            if not np.allclose(T, np.eye(4)):
                V = mesh.vertices
                mesh.vertices = (np.c_[V, np.ones(len(V))] @ T.T)[:, :3]
                if np.linalg.det(T[:3, :3]) < 0:
                    mesh.faces = [tuple(reversed(f)) for f in mesh.faces]
                try:
                    tree = tree.translate(tuple(float(x) for x in T[:3, 3])) if np.allclose(T[:3, :3], np.eye(3)) else None
                except Exception:
                    tree = None
            mat = obj.material
            name = _default_mat_name()
            if mat is not None and hasattr(mat, "base_color"):
                # the same colour vocabulary /api/semantic uses ("red" -> ruby), so the two doors agree;
                # metals and anything off-vocabulary fall to the nearest library preset (reported)
                rgb = np.asarray(mat.base_color[:3], float)
                metallic = float(getattr(mat, "metallic", 0.0))
                word = _nearest_named_color(rgb)
                name = _COLOR_TO_MAT.get(word) if metallic < 0.5 else None
                if not name:
                    name = _nearest_preset(rgb, metallic, float(getattr(mat, "roughness", 0.5)))
            # the document's own SDF classes (_SphereSDF ...) have .eval but not the DSL contract
            # (.to_dsl / .translate) every downstream consumer of sdf_tree assumes -- keep the tree only
            # when it IS the engine's SDF DSL, else the object is mesh-only (honest: no shader export)
            keep = tree if (tree is not None and hasattr(tree, "to_dsl") and hasattr(tree, "translate")) else None
            nid = _add_object(str(obj.name or h), mesh, [name] * mesh.n_faces, sdf_tree=keep)
            made.append({"object": nid, "handle": h, "name": obj.name, "material": name,
                         "geometry": type(geo).__name__.strip("_")})
        if made:
            _bump()
        out = _payload(view="summary") if made else {}
    out.update({"made": made, "unknown": _json_safe(r.get("unknown", [])),
                "suggestions": _json_safe(r.get("suggestions", [])),
                "handles": _json_safe(r.get("handles", {}))})
    return jsonify(out), (200 if made else 400)


@bp.route("/api/scene/animate", methods=["POST"])
def scene_animate():
    """m.render_animation on the Scene document: {keys: {<object id or handle>: {position: [[t,[x,y,z]],..],
    rotation: ..., scale: ...}}, n_frames, fps, w, h, eye, target} -> an animated GIF (data URL) plus the
    frame count. Keyframes are in seconds; the GIF is the engine's deterministic GIF89a writer."""
    _init()
    d = request.get_json(force=True) or {}
    keys_in = d.get("keys") or {}
    if not isinstance(keys_in, dict) or not keys_in:
        return jsonify({"error": "need keys: {object: {position|rotation|scale: [[t, value], ...]}}"}), 400
    with _LOCK:
        s = _scene_doc()
        by_oid = {oid: h for h, oid in _DOC["handles"].items()}
        keys = {}
        for k, v in keys_in.items():
            h = by_oid.get(str(k), k)
            if h not in _DOC["handles"]:
                return jsonify({"error": f"no object or handle {k!r}"}), 400
            keys[h] = v
        cam, W, H = _doc_camera(d)
        n = int(np.clip(int(d.get("n_frames", 12)), 2, 96))
        import tempfile
        fd, path = tempfile.mkstemp(suffix=".gif"); os.close(fd)
        try:
            frames = _mind().render_animation(s, cam, keys, n_frames=n, fps=float(d.get("fps", 12.0)),
                                              width=W, height=H, gif=path)
            with open(path, "rb") as f:
                gif = f.read()
        except Exception as e:
            return jsonify({"error": f"render_animation refused: {type(e).__name__}: {e}"}), 400
        finally:
            try:
                os.unlink(path)
            except OSError:
                pass
    return jsonify({"frames": len(frames), "w": W, "h": H,
                    "gif": "data:image/gif;base64," + base64.b64encode(gif).decode()})


@bp.route("/api/workspace/gc", methods=["POST"])
def workspace_gc():
    """`Workspace.gc_assets`: drop content-addressed assets nothing references. {"dry_run": true} lists only."""
    ws = _workspace()
    if ws is None:
        return jsonify({"error": "no live workspace"}), 400
    d = request.get_json(force=True, silent=True) or {}
    try:
        return jsonify({"dropped": _json_safe(ws.gc_assets(dry_run=bool(d.get("dry_run", False))))})
    except Exception as e:
        return jsonify({"error": f"{type(e).__name__}: {e}"}), 400


@bp.route("/api/workspace/note", methods=["POST"])
def workspace_note():
    """`lews_note`: a non-section change (a selection, a viewport camera) as ONE journal line, no rewrite --
    how other participants see what you are looking at. {kind, data}; identity from X-User."""
    ws = _workspace()
    if ws is None:
        return jsonify({"error": "no live workspace"}), 400
    d = request.get_json(force=True, silent=True) or {}
    who = request.headers.get("X-User") or "anonymous"
    try:
        rev = _mind().lews_note(_ws_root(), who, str(d.get("kind", "note")), d.get("data") or {})
    except Exception as e:
        return jsonify({"error": f"{type(e).__name__}: {e}"}), 400
    return jsonify({"rev": rev})


@bp.route("/api/workspace/wait")
def workspace_wait():
    """`lews_wait`: long-poll for the next change after ?rev= (timeout ?t=, max 25 s) -- the primitive an SSE
    stream is one loop around, for clients that cannot hold an EventSource."""
    ws = _workspace()
    if ws is None:
        return jsonify({"error": "no live workspace"}), 400
    rev = int(request.args.get("rev", 0)); t = float(np.clip(float(request.args.get("t", 10)), 0.1, 25.0))
    try:
        return jsonify({"changes": _json_safe(_mind().lews_wait(_ws_root(), rev, timeout=t))})
    except Exception as e:
        return jsonify({"error": f"{type(e).__name__}: {e}"}), 400
...
def invite():
    """The invite button (m.create_invite_link): a shareable link + bare code that lets a guest join this
    session read-only. {base_url?, grants?}."""
    d = request.get_json(force=True, silent=True) or {}
    try:
        inv = _mind().create_invite_link(workspace=_ws_root() or "lestudio3d",
                                         base_url=str(d.get("base_url") or request.host_url), grants=d.get("grants"))
    except Exception as e:
        return jsonify({"error": f"{type(e).__name__}: {e}"}), 400
    return jsonify(_json_safe(inv))


@bp.route("/api/join", methods=["POST"])
def join():
    """The join button (m.join_from_link): {link_or_code} + X-User -> the scoped guest principal."""
    d = request.get_json(force=True, silent=True) or {}
    who = request.headers.get("X-User") or str(d.get("user") or "guest")
    try:
        g = _mind().join_from_link(str(d.get("link_or_code", "")), who, workspace=_ws_root() or "lestudio3d")
    except Exception as e:
        return jsonify({"error": f"{type(e).__name__}: {e}"}), 400
    return jsonify({"principal": _json_safe(getattr(g, "id", g)), "who": who})


# ---------------------------------------------------------------- THE WHOLE ENGINE, beside the app
# APP_FOUNDATION §3: "For things an agent should read but not call through the app, the engine's own service
# (/tools + /invoke) already exposes every faculty; mount it beside the app rather than proxying it." This is
# that: holographic_service.Service on THIS process's mind, every route it has (tools, invoke with object
# handles, jobs, documents, skills, capabilities search, bus, sql), under /engine/. The app's own Scene document
# is registered as a handle, so an agent can drive it with any of the ~2,400 faculties, not only our routes.
_SERVICE = {"svc": None}


def _service():
    if _SERVICE["svc"] is None:
        import holographic_service as HS
        _SERVICE["svc"] = HS.Service(mind=_mind())
    return _SERVICE["svc"]


def mount_engine_service(app, base="/engine"):
    """Every (method, path) the engine service answers, as Flask routes under `base`. Streams are skipped
    (they are HTTP-server generators; use the engine's own serve.sh for those)."""
    svc = _service()

    def make(method, path, handler):
        def view():
            payload = (request.get_json(force=True, silent=True) or {}) if method == "POST" else dict(request.args)
            try:
                out = handler(payload)
            except Exception as e:                          # a caller error is a 400 with the reason, never a 500
                return jsonify({"ok": False, "error": f"{type(e).__name__}: {e}"}), 400
            if hasattr(out, "__next__"):
                return jsonify({"ok": False, "error": "streaming route; use the engine's own service"}), 400
            try:
                return Response(json.dumps(out, default=_json_safe), mimetype="application/json")
            except Exception:
                return jsonify(_json_safe(out))
        view.__name__ = "engine_" + method.lower() + path.replace("/", "_")
        view.__doc__ = (getattr(handler, "__doc__", "") or "").strip().split("\n")[0] or f"engine {path}"
        app.add_url_rule(base + path, endpoint="engine" + path.replace("/", "_") + "_" + method, view_func=view,
                         methods=[method])
    ...
    for (method, path), handler in sorted(svc._routes.items()):
        if path.endswith("/stream"):
            continue
        make(method, path, handler); n += 1
    return n


def _scene_doc_ref():
    """Register this scene's engine Scene document with the service's object registry -> 'ref:Scene:N', usable
    as the `scene` argument of any /engine/invoke faculty."""
    s = _scene_doc()
    return _service().refs.put(s)


# ---------------------------------------------------------------- the mount
_FOUNDATION = {"mounted": False, "have": {}}
...
def _agent_hints(app):
    """One summary per route for the engine's manifest: our hint table first, else the FIRST SENTENCE of
    the view's docstring (the engine takes the first LINE, which for a wrapped docstring stops mid-clause --
    the render_api_test rule), else a readable name. Handed to agent_surface(hints=)."""
    out = {}
    for rule in app.url_map.iter_rules():
        path = str(rule)
        if not path.startswith("/api/") or "/agent/" in path or "<" in path:
            continue
        name = path[len("/api/"):].rstrip("/")
        view = app.view_functions.get(rule.endpoint)
        try:
            out[name] = _tool_summary(name, view)
        except Exception:
            continue
    return out


def mount_foundation(app, workspace_root=None, verbose=True):
    """Mount the engine's foundation on a Flask app that registered `bp`: one mind, features() gating,
    the live .lews (LESTUDIO3D_WORKSPACE), our private kind registered, and the standard agent surface
    (tools/invoke/mind/engine/events/presence). Idempotent. Returns the features dict."""
    if _FOUNDATION["mounted"]:
        return _FOUNDATION["have"]
    mind = _mind()
    have = mind.features(["agent_surface", "lews_open", "render_quality_gate", "mesh_uv_unwrap",
                          "mesh_catmull_clark", "mesh_to_sdf_grid", "describe_to_scene", "render_preview",
                          "render_animation", "fit_camera", "surface_retopo", "cvt_remesh", "manifold_cleanup",
                          "mesh_orient", "studio_sky", "sky_model", "load_exr", "app_substrate",
                          "lews_preset_section", "lews_mint"])
    _FOUNDATION.update(mounted=True, have=have)
    log = print if verbose else (lambda *a, **k: None)
    log("  [engine] " + str(mind.engine_status().get("engine", "?")) +
        "  missing: " + (", ".join(k for k, v in have.items() if not v) or "nothing this app calls"))
    root = workspace_root or _ws_root()
    try:
        from holographic.io_and_interop import holographic_lews as _L
        _L.register_kind_schema("lestudio3d.scene", 1, describe="leStudio3d's full-fidelity scene (the JSON "
                                "File > Save writes: objects, custom materials, node graph, render assets, units)")
    except Exception:
        pass
    if root:
        os.environ["LESTUDIO3D_WORKSPACE"] = root
        ws = _workspace()
        log(f"  [lews] live workspace {root}" if ws is not None else "  [lews] workspace unavailable")
    if have.get("agent_surface"):
        mind.agent_surface(app, base="/api", app_name="lestudio3d", workspace_root=root,
                           image_routes=("render", "render_progressive", "render_engine", "object_texture",
                                         "scene_preview"),
                           stream_routes=("photo",), hints=_agent_hints(app))
        log("  [agent] /api/agent/{tools,invoke} + /api/{mind,engine,events,presence}")
    else:
        log("  [agent] engine has no agent_surface; upgrade leos-core")
    try:
        n = mount_engine_service(app)
        log(f"  [engine] whole engine mounted at /engine/ ({n} routes; POST /engine/invoke runs any faculty)")
    except Exception as e:
        log(f"  [engine] service mount skipped ({type(e).__name__}: {e})")
    return have
