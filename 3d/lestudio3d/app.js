'use strict';
/* leStudio3d editor — leCore is the authority; this file is the viewport, the tools, and the wire.
...
async function doPreview(){
  ...
  try{
    ...
    if(previewWanted()){
      ...
      const _hdr=r.headers.get('X-LeCore-Render')||'';
...
function rvSave(){
  ...
  const a=document.createElement('a'); a.href=img.src; a.download=`lestudio3d_${kind}_${rvStamp()}.png`; a.click();
...
(function rvWire(){
  ...
  grip.addEventListener('pointerdown', ev=>{
    ...
    const up=()=>{ removeEventListener('pointermove',mv); removeEventListener('pointerup',up);
      try{ localStorage.setItem('lestudio3d:rvsize', JSON.stringify({w:dlg.style.width,h:$('rvstage').style.height})); }catch(_){ } };
    addEventListener('pointermove',mv); addEventListener('pointerup',up); ev.preventDefault();
  });
  try{ const sz=JSON.parse(localStorage.getItem('lestudio3d:rvsize')||'null');
...
/* 1.6.0: the engine's remeshers (surface_retopo / cvt_remesh / manifold_cleanup / mesh_orient) as Mesh ops */
async function runReportedOp(op, args){
  status(op.replace(/_/g,' ')+'…');
  try{
    const r = await api('op', Object.assign({op, object:ACTIVE}, args||{}));
    if(r && r.error){ status(r.error); return; }
    applyResp(r);
    const f=r.faces||{}, q=(r.report||{}).quad_fraction;
    status(op.replace(/_/g,' ')+`: ${f.before} → ${f.after} faces`+(q!=null?` · ${Math.round(q*100)}% quads`:''));
  }catch(e){ status(op+' failed.'); }
}
$('tRetopo').onclick=async()=>{ if(!ACTIVE) return; const v=await uiPrompt('Retopologise','Density (0.2–4; 1 = about the source\'s edge length)','1'); if(v==null) return; runReportedOp('retopo',{density:+v||1}); };
$('tCvt').onclick=async()=>{ if(!ACTIVE) return; const v=await uiPrompt('CVT remesh','Sites (target vertex count, 20–20000)','400'); if(v==null) return; runReportedOp('cvt_remesh',{sites:+v||400}); };
$('tManifold').onclick=()=>{ if(ACTIVE) runReportedOp('manifold_cleanup',{}); };
$('tOrient').onclick=()=>{ if(ACTIVE) runReportedOp('orient',{}); };
/* 1.6.0: words -> the engine's Scene document -> objects (describe_to_scene) */
$('tDescribeDoc').onclick=async()=>{
  const t=await uiPrompt('Describe a scene','e.g. "a red cube next to a green sphere, and a gold torus"','');
  if(!t) return;
  status('Describing…');
  try{
    const r=await api('scene/describe',{text:t});
    if(r.error){ status(r.error+(r.unknown&&r.unknown.length?' — unknown: '+r.unknown.join(', '):'')); return; }
    applyResp(r);
    status(`Made ${r.made.length} object(s): `+r.made.map(m=>`${m.name} (${m.material})`).join(', ')+(r.unknown&&r.unknown.length?` · not understood: ${r.unknown.join(', ')}`:''));
  }catch(e){ status('Describe failed.'); }
};
...
$('nodeclear').onclick = async()=>{
  ...
  if(n>0){
    ...
    try{
      ...
      a.download = 'lestudio3d_graph_backup.json'; a.click(); URL.revokeObjectURL(a.href);
    }catch(e){}
  }
...
async function rvTurntable(){
  ...
  if(frames.length){
    ...
    a.href = URL.createObjectURL(blob); a.download = `lestudio3d_turntable_${rvStamp()}.zip`; a.click();
  ...
  document.querySelectorAll('.dlg').forEach(d=>{
    ...
    grip.addEventListener('pointerdown', ev=>{
      ...
      const up=()=>{
        ...
        try{ const all=JSON.parse(localStorage.getItem('lestudio3d:dlgsize')||'{}');
             all[d.id]={w:d.style.width, h:body.style.maxHeight};
             localStorage.setItem('lestudio3d:dlgsize', JSON.stringify(all)); }catch(_){ }
      };
      addEventListener('pointermove',mv); addEventListener('pointerup',up); ev.preventDefault();
    });
  });
  try{
    const all=JSON.parse(localStorage.getItem('lestudio3d:dlgsize')||'{}');
...
(function askUI(){
  ...
  // info: a read-only panel built by the caller; resolves when closed
  window.uiInfo = (title, buildBody)=>open(title, buildBody, ()=>true, 'Close', false);
...
(async function boot(){
  ...
  try{ if($('welcome') && localStorage.getItem('lestudio3d:welcome')!=='seen') $('welcome').style.display='block'; }
...
$('cm_save').onclick=async()=>{
  ...
  try{
    ...
    MATCLASS='custom'; MATNAME=d.name; buildSwatches(); buildMatbar();
...
function buildMatbar(){
  ...
  if(hideBtn) hideBtn.onclick = (e)=>{ e.stopPropagation(); document.body.classList.add('matbar-hidden');
    try{ localStorage.setItem('lestudio3d:matbar','hidden'); }catch(_){}} ;
...
$('secbtn') && ($('secbtn').onclick = async ()=>{
  ...
  try{
    ...
    $('secmeta').textContent = r.headers.get('X-LeCore-Section')||'';
...
$('scenesave') && ($('scenesave').onclick = async ()=>{
  try{
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
    a.download = 'lestudio3d_scene.json'; a.click();
...
$('welcomeclose') && ($('welcomeclose').onclick = ()=>{ if($('welcome')) $('welcome').style.display='none';
  try{ localStorage.setItem('lestudio3d:welcome','seen'); }catch(_){ } });
...
$('refunderlay') && ($('refunderlay').addEventListener('click', async (ev)=>{
  ...
  try{
    ...
    MATS = await api('materials'); buildSwatches(); buildMatbar();
...
$('tlRec') && ($('tlRec').onclick = ()=>{
  ...
  _rec.onstop=()=>{
    ...
    a.download='lestudio3d_anim.webm'; a.click();
...
const GEN_SPECS = {
  ...
  studio: {title:'Studio lighting rig', note:'The engine\'s three-point rig as an environment (m.studio_sky): warm key, cool weak fill, rim behind. It becomes the render sky AND the GI dome light.',
    params:[{id:'preset',label:'Rig',type:'select',options:['classic','soft','dramatic']},
            {id:'gain',label:'Gain',type:'range',min:0.2,max:4,step:0.1,val:1},
            {id:'quality',label:'Quality',type:'select',options:['dome','high']}]},
  sky: {title:'Parametric sky', note:'The engine\'s sky model (m.sky_model): hour drives the gradient and the sun\'s arc; clouds by kind and cover; stars by seed.',
    params:[{id:'hour',label:'Hour',type:'range',min:0,max:24,step:0.5,val:12},
            {id:'cloud',label:'Cloud kind',type:'select',options:['none','cirrus','cirrostratus','cirrocumulus','altocumulus','altostratus','stratocumulus','nimbostratus']},
            {id:'cover',label:'Cloud cover',type:'range',min:0,max:1,step:0.05,val:0.4},
            {id:'moon',label:'Moon',type:'check',val:false},
            {id:'quality',label:'Quality',type:'select',options:['dome','high']}]},
...
$('genGo') && ($('genGo').onclick = async ()=>{
  ...
  try{
    if(GEN_KIND==='env' || GEN_KIND==='studio' || GEN_KIND==='sky'){
      if(GEN_KIND==='studio'){ body.studio=body.preset; delete body.preset; body.gain=+body.gain; }
      if(GEN_KIND==='sky'){ body.sky={hour:+body.hour, clouds: body.cloud&&body.cloud!=='none' ? [[body.cloud,+body.cover]] : [], moon:!!body.moon, seed:body.seed}; }
      ...
      $('genMeta').innerHTML='Backdrop set — it now lights GI photos too. <a class="dl" id="envdl" href="data:image/png;base64,'+r.preview+'" download="lestudio3d_env.png">download PNG</a>';
      status('Backdrop: '+(body.preset||body.studio||('sky '+(body.sky&&body.sky.hour)+'h'))+'.');
    }else{
      body.kind=GEN_KIND;
...
const STORAGE_PREFIX = 'lestudio3d:scene:';
...
try{ if(localStorage.getItem('lestudio3d:matbar')==='hidden') document.body.classList.add('matbar-hidden'); }catch(_){}
$('matbarshow') && ($('matbarshow').onclick = ()=>{ document.body.classList.remove('matbar-hidden');
  try{ localStorage.setItem('lestudio3d:matbar','shown'); }catch(_){} });
...
/* 1.6.0: the engine's reading of this scene (scene_info) and the shared preset list */
$('scenedoc') && ($('scenedoc').onclick = async ()=>{
  try{
    const d=await (await fetch('api/scene/doc')).json();
    const rows=(d.objects||[]).map(o=>`<div class="sub"><b>${o.name}</b> — ${o.geometry} · ${o.material||'default'} · id ${(d.handles||{})[o.handle]||'?'} · handle ${o.handle}</div>`).join('');
    await uiInfo('Scene as the engine reads it', b=>{ b.innerHTML=`<div class="sub">${d.n_objects} object(s)${d.problems&&d.problems.length?` · problems: ${d.problems.join('; ')}`:' · no pre-flight problems'}</div>`+rows; });
  }catch(e){ status('Could not read the scene document.'); }
});
async function sceneDirty(){ try{ const fresh=await (await fetch('api/scene')).json(); applyResp(fresh,{force:true}); }catch(_){ } }
/* the whole engine: search the capability index, invoke any faculty with JSON args, see the result */
$('engineconsole') && ($('engineconsole').onclick = async ()=>{
  let sref=null; try{ const d=await (await fetch('api/scene/doc')).json(); sref=d.ref||null; }catch(_){ }
  await uiInfo('Engine console — POST /engine/invoke', b=>{
    b.innerHTML=`<div class="sub">Every leCore faculty (~2,400) is mounted at <code>/engine/</code>. Search, pick, pass JSON args; object results come back as <code>ref:Type:N</code> handles you can pass on.${sref?` This scene's document is <code>${sref}</code> (use it as <code>scene</code>).`:''}</div>
      <div class="inline"><input id="ec_q" type="text" placeholder="search capabilities, e.g. cvt remesh" style="flex:1"><button id="ec_find">Find</button></div>
      <div id="ec_hits" class="sub" style="max-height:120px;overflow:auto"></div>
      <div class="inline">Faculty <input id="ec_name" type="text" style="flex:1" placeholder="e.g. mass_properties"></div>
      <div class="inline">Args (JSON) <input id="ec_args" type="text" style="flex:1" value="{}"></div>
      <div class="inline"><button id="ec_run">Invoke</button><label class="inline" style="margin-left:8px"><input type="checkbox" id="ec_budget" checked> bound big results (budget)</label></div>
      <pre id="ec_out" style="max-height:220px;overflow:auto;white-space:pre-wrap;font-size:11px"></pre>`;
    const q=b.querySelector('#ec_q'), hits=b.querySelector('#ec_hits'), nm=b.querySelector('#ec_name'), ar=b.querySelector('#ec_args'), out=b.querySelector('#ec_out');
    b.querySelector('#ec_find').onclick=async()=>{
      hits.textContent='searching…';
      try{ const r=await (await fetch('engine/capabilities/search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:q.value,k:8})})).json();
        const list=r.results||r.hits||r.capabilities||[];
        hits.innerHTML=list.map(c=>{ const n=c.name||c.faculty||c.entry||''; const e=(c.faculties||c.methods||[])[0]||''; return `<div><a href="#" data-n="${e||n}"><b>${n}</b></a> — ${(c.does||c.summary||c.description||'').slice(0,110)}</div>`; }).join('')||JSON.stringify(r).slice(0,300);
        hits.querySelectorAll('a').forEach(a=>a.onclick=e=>{ e.preventDefault(); nm.value=a.dataset.n; });
      }catch(e){ hits.textContent='search failed: '+e.message; }
    };
    b.querySelector('#ec_run').onclick=async()=>{
      let args={}; try{ args=JSON.parse(ar.value||'{}'); }catch(e){ out.textContent='args are not JSON: '+e.message; return; }
      out.textContent='running…';
      const body={name:nm.value.trim(), args}; if(b.querySelector('#ec_budget').checked) body.budget=20000;
      try{ const r=await (await fetch('engine/invoke',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})).json();
        out.textContent=JSON.stringify(r,null,1).slice(0,20000);
        if(r.ok && nm.value.trim().startsWith('scene')) sceneDirty&&sceneDirty();
      }catch(e){ out.textContent='invoke failed: '+e.message; }
    };
  });
});
/* the .lews workspace panel: presence, kinds, feed, invite */
$('wspanel') && ($('wspanel').onclick = async ()=>{
  try{
    const d=await (await fetch('api/workspace?since=0')).json();
    await uiInfo('Workspace (.lews)', b=>{
      if(!d.open){ b.innerHTML=`<div class="sub">${d.hint||'no live workspace'}</div>`; return; }
      const who=(d.presence||[]).map(p=>`<div class="sub">${p.host?'★ ':''}<b>${p.name||p.who}</b> — ${p.app}${p.activity&&p.activity.tool?' · '+p.activity.tool:''}</div>`).join('')||'<div class="sub" style="color:var(--dim)">nobody else yet</div>';
      const feed=(Array.isArray(d.changes)?d.changes.slice(-12):[]).map(e=>`<div class="sub">rev ${e.rev} · ${e.app||''} · ${e.kind||e.op||''} ${e.id||''}</div>`).join('');
      b.innerHTML=`<div class="sub"><b>${d.root}</b> · rev ${(d.describe||{}).rev??'?'} · kinds: ${(d.kinds||[]).join(', ')||'none'} · assets: ${d.assets??'?'}</div>
        <div class="mglabel">Who is here</div>${who}
        <div class="mglabel">Recent changes</div><div style="max-height:140px;overflow:auto">${feed||'<div class="sub">none</div>'}</div>
        <div class="inline"><button id="ws_inv">Create invite link</button><input id="ws_join" type="text" placeholder="paste a link or code to join" style="flex:1"><button id="ws_joinb">Join</button></div>
        <pre id="ws_out" style="white-space:pre-wrap;font-size:11px"></pre>`;
      b.querySelector('#ws_inv').onclick=async()=>{ const r=await (await fetch('api/invite',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})).json(); b.querySelector('#ws_out').textContent=r.error||('link: '+r.link+'\ncode: '+r.code); };
      b.querySelector('#ws_joinb').onclick=async()=>{ const r=await (await fetch('api/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({link_or_code:b.querySelector('#ws_join').value})})).json(); b.querySelector('#ws_out').textContent=r.error||('joined as '+r.principal); };
    });
  }catch(e){ status('Could not read the workspace.'); }
});
$('lewsgc') && ($('lewsgc').onclick = async ()=>{
  try{ const r=await (await fetch('api/workspace/gc',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})).json(); status(r.error||`Collected ${(r.dropped||[]).length} unreferenced asset(s).`); }catch(e){ status('GC failed.'); }
});
$('presetsBtn') && ($('presetsBtn').onclick = async ()=>{
  try{
    const d=await (await fetch('api/presets')).json();
    const rows=(d.presets||[]).map(p=>`<div class="sub"><b>${p.name}</b> — ${p.target} <span style="color:var(--dim)">(${p.app||'?'}${p.author?' · '+p.author:''})</span> <code>${JSON.stringify(p.params)}</code></div>`).join('')
      || '<div class="sub" style="color:var(--dim)">no presets yet — the Render settings and Material editor can save one</div>';
    await uiInfo('Presets', b=>{ b.innerHTML=`<div class="sub">${d.workspace?'shared through the live .lews workspace (visible to leStudio)':'this session only — set LESTUDIO3D_WORKSPACE to share'}</div>`+rows; });
  }catch(e){ status('Could not list presets.'); }
});
/* 1.6.0: pull painted documents + lecore.mesh sections straight from the LIVE workspace (no file) */
$('lewspull') && ($('lewspull').onclick = async ()=>{
  status('Pulling from the live workspace…');
  try{
    const target = ACTIVE ? `?object=${encodeURIComponent(ACTIVE)}` : '';
    const r=await fetch('api/workspace/import'+target,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({from:'workspace'})});
    const d=await r.json(); if(!r.ok) throw new Error(d.error||r.status);
    applyResp(d); const w=d.workspace||{};
    status(`Pulled ${(w.textures||[]).length} texture(s), ${(w.imported_meshes||[]).length} mesh(es)`+(w.applied_to?' — texture applied to the selected object':''));
  }catch(e){ status('Pull failed: '+e.message); }
});
/* 1.6.0: Render view -- engine fast preview and exact framing */
$('rvquick') && ($('rvquick').onclick = async ()=>{
  status('Quick look…');
  try{
    const r=await fetch('api/scene_preview?w=480&h=360&quality=draft'); if(!r.ok) throw new Error(r.status);
    const url=URL.createObjectURL(await r.blob());
    rvShow(url,'final',{label:'quick look (engine render_preview)'}); status('Quick look ready.');
  }catch(e){ status('Quick look failed: '+e.message); }
});
$('rvframe') && ($('rvframe').onclick = async ()=>{
  try{
    const c=await (await fetch('api/camera/fit?w=640&h=360')).json(); if(c.error) throw new Error(c.error);
    // the orbit is spherical (theta/phi/dist about camTarget): convert the engine's eye/target into it
    camTarget.set(c.target[0],c.target[1],c.target[2]);
    const dx=c.eye[0]-c.target[0], dy=c.eye[1]-c.target[1], dz=c.eye[2]-c.target[2];
    camDist=Math.max(0.2, Math.hypot(dx,dy,dz)); camPhi=Math.acos(Math.max(-1,Math.min(1,dy/camDist))); camTheta=Math.atan2(dz,dx);
    applyCam(); scheduleRender(); status('Framed all objects (engine fit_camera).');
  }catch(e){ status('Frame all failed: '+e.message); }
});
$('enginestatus') && ($('enginestatus').onclick = async ()=>{
  try{
    const d = await (await fetch('api/engine_status')).json();
    const ex = Object.entries(d.extras||{}).map(([k,v])=>`${(v&&v.ok!==false&&v!==false)?'\u2713':'\u2014'} ${k}${(v&&v.reason)?' \u2014 '+v.reason:''}`).join('\n');
    const feat = (d.missing||[]).length ? `MISSING faculties this app calls: ${d.missing.join(', ')}\n(pip install -U "leos-core[ui]")` : `Every faculty this app calls is present (${Object.keys(d.features||{}).length} checked).`;
    const pol = d.policy ? `\nDeterminism: bit-exact ${d.policy.bit_exact}` + (d.policy.notes?` \u2014 ${[].concat(d.policy.notes).join('; ')}`:'') : '';
    uiReport('Engine status', `Engine: ${d.engine||'?'}  faculties: ${d.faculties||d.faculty_count||'?'}\nWorkspace: ${d.workspace||'(none \u2014 set LESTUDIO3D_WORKSPACE)'}\n\n${feat}${pol}\n\nOptional extras:\n${ex}`);
