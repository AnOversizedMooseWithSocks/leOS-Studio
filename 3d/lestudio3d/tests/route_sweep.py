...
Runs against the REAL engine (leos-core is a hard dependency), so every 500 is the app's -- there is no
stub to blame. Routes that need a parameter refuse with 4xx; that is the contract.
...
app = Flask(__name__); app.register_blueprint(backend.bp); backend.mount_foundation(app, verbose=False)
