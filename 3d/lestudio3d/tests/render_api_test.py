"""Call the render routes over real HTTP, against the real engine (leos-core is a hard dependency).
...
def serve():
    ...
    backend.mount_foundation(app, verbose=False)   # one mind + the agent surface, as app.py mounts it
    port = free_port()
...
def main():
    ...
    import holographic.rendering.holographic_pathtrace as fake_pt
    ...
    check("/api/render reports its stats header", bool(r.headers.get("X-LeCore-Render")),
          "missing X-LeCore-Render (the client parses this for warnings and perf)")
    ...
    png = r.read()
    hdr = r.headers.get("X-LeCore-Render") or ""
